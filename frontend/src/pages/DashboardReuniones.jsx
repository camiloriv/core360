...
export default function DashboardReuniones() {
  ...
  const [filters, setFilters] = useState({ empresa: "Todas", ejecutiva: "Todas", jefatura: "Todas", tipo: "Todas" });
  const [options, setOptions] = useState({ empresas: ["Todas"], ejecutivas: ["Todas"], jefaturas: ["Todas"], tipos: ["Todas"] });
    ...
    const fetchData = async () => {
      try {
        const [reunRes, statsRes] = await Promise.all([
          ...
          axios.get("http://localhost:8080/reuniones/stats")
        ]);
        ...
        const emps = ["Todas", ...new Set(reunRes.data.map(r => r.empresa_nombre).filter(Boolean))].sort();
        const ejes = ["Todas", ...new Set(reunRes.data.map(r => r.ejecutiva_nombre).filter(Boolean))].sort();
        const jefs = ["Todas", ...new Set(reunRes.data.map(r => r.jefatura_nombre).filter(Boolean))].sort();
        const tips = ["Todas", ...new Set(reunRes.data.map(r => r.tipo_reu).filter(Boolean))].sort();
        setOptions({ empresas: emps, ejecutivas: ejes, jefaturas: jefs, tipos: tips });
  ...
  const availableEjecutivas = ["Todas", ...[...new Set(
    reuniones
      .filter(r => filters.jefatura === "Todas" || r.jefatura_nombre === filters.jefatura)
      .map(r => r.ejecutiva_nombre)
      .filter(Boolean)
  )].sort()];

  const availableEmpresas = ["Todas", ...[...new Set(
    reuniones
      .filter(r => (filters.jefatura === "Todas" || r.jefatura_nombre === filters.jefatura) && 
                   (filters.ejecutiva === "Todas" || r.ejecutiva_nombre === filters.ejecutiva))
      .map(r => r.empresa_nombre)
      .filter(Boolean)
  )].sort()];

  const availableTipos = ["Todas", ...[...new Set(reuniones.map(r => r.tipo_reu).filter(Boolean))].sort()];
  const availableJefaturas = ["Todas", ...[...new Set(reuniones.map(r => r.jefatura_nombre).filter(Boolean))].sort()];

  const filteredReuniones = reuniones.filter(r => {
    return (filters.empresa === "Todas" || r.empresa_nombre === filters.empresa) &&
           (filters.ejecutiva === "Todas" || r.ejecutiva_nombre === filters.ejecutiva) &&
           (filters.jefatura === "Todas" || r.jefatura_nombre === filters.jefatura) &&
    ...
    <div className="encuesta-page" style={{ background: '#f8fafc', minHeight: '100vh' }}>
      <div className="container">
        ...
        <div style={{ marginBottom: '30px' }}>
          <h2 className="title" style={{ marginBottom: '5px' }}>Dashboard de Gestión de Reuniones</h2>
          <p className="subtitle" style={{ marginTop: '0' }}>ANÁLISIS DE ACTIVIDAD Y MINUTAS ESTRATÉGICAS</p>
        </div>
        ...
        <div style={{ 
          ...
          <div>
            <label style={{ ...styles.label, display: 'block', marginBottom: '8px', fontSize: '14px', fontWeight: 'bold' }}>EMPRESA</label>
            <SearchableFilter 
              label=""
          ...
          <div>
            <label style={{ ...styles.label, display: 'block', marginBottom: '8px', fontSize: '14px', fontWeight: 'bold' }}>JEFATURA</label>
            <select 
              style={{ ...styles.select, width: '100%' }} 
              value={filters.jefatura} 
              onChange={e => setFilters({...filters, jefatura: e.target.value, ejecutiva: "Todas", empresa: "Todas"})}
            >
              {availableJefaturas.map(opt => <option key={opt} value={opt}>{opt}</option>)}
            </select>
          </div>
        </div>
        ...
        <div style={{ 
          ...
          <KpiCard 
            ...
            value={filteredReuniones.filter(r => r.lugar === 'Presencial').length} 
          ...
          <KpiCard 
            title="Este Mes" 
            value={filteredReuniones.filter(r => {
          ...
          <div style={{ ...styles.chartBox, padding: 0, overflow: 'hidden', border: '1px solid #e2e8f0' }}>
            ...
            <div style={{ padding: '20px', height: '350px', display: 'flex', flexDirection: 'column' }}>
                ...
                <PieChart>
                  ...
                  >
                    {barData.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={getMeetingColor(entry.name)} />
                    ))}
                  </Pie>
                  <Tooltip />
                </PieChart>
              ...
              <div style={{ 
                ...
                {barData.map((entry, index) => {
                  const total = barData.reduce((acc, curr) => acc + curr.total, 0);
                  const percent = ((entry.total / total) * 100).toFixed(1);
                  return (
                    <div key={index} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <div style={{ width: '12px', height: '12px', borderRadius: '50%', background: getMeetingColor(entry.name) }}></div>
                      <div style={{ flex: 1, display: 'flex', justifyContent: 'space-between' }}>
                        <span>{entry.name} ({entry.total})</span>
            ...
            <table style={styles.table}>
              ...
              <tbody>
                {filteredReuniones.map((r) => {
                  const adjuntos = r.archivos_nombres ? JSON.parse(r.archivos_nombres) : [];
                  return (
                    <tr key={r.id_reunion} style={styles.tr}>
                      <td style={styles.tdCell}>
                        <div style={styles.companyName}>{new Date(r.fecha_reu).toLocaleDateString()}</div>
                        <div style={styles.meetingIdText}>{r.id_reunion}</div>
                      </td>
                      <td style={styles.tdCell}>
                        <div style={styles.companyName}>{r.empresa_nombre}</div>
                        <div style={styles.ejecutivaName}>{r.ejecutiva_nombre}</div>
                      </td>
                      <td style={styles.tdCell}>
                      ...
                      <td style={styles.tdCell}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '5px', marginTop: '4px' }}>
                          <span style={{ 
                            ...
                            background: r.estado_envio === 'enviado' ? '#dcfce7' : '#fef9c3',
                            color: r.estado_envio === 'enviado' ? '#166534' : '#854d0e'
                          }}>
                        ...
                        {r.programar_encuesta ? (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                            <span style={{ 
                              ...
                              color: r.encuesta_estado_envio === 'enviado' ? '#166534' : '#0369a1'
                            }}>
