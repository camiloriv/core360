...
// ­ƒö╣ Funci├│n auxiliar para DRY: Construye WHERE seg├║n el rol
...
// ­ƒö╣ ESTAD├ìSTICAS PARA DASHBOARD
...
exports.crearReunion = async (req, res) => {
    ...
    if (totalSize > MAX_SIZE) {
        return res.status(400).json({
            error: "El tama├▒o total de los archivos adjuntos supera el l├¡mite permitido de 20MB."
        });
    }
        ...
        if (result2.length > 0) {
            ...
            try {
                ...
                enviarCorreo({
                    to: data.enviado_a,
                    cc: correosCc,
                    subject: `Reuni├│n ${data.tipo_reu} - ${data.empresa_nombre} - ${data.id_reunion}`,
                    data: {
                ...
                console.error("Error al preparar env├¡o de correo:", error);
            }
        }

        res.json({
            msg: "Reuni├│n creada y minuto enviado",
            id_reunion
        });

    } catch (error) {
        console.error("Error al crear reuni├│n:", error);
        return res.status(500).json({ error: "Error interno al crear la reuni├│n" });
    }
};
...
// ­ƒö╣ OBTENER TIPOS DE REUNI├ôN UNICOS
...
// ­ƒö╣ PROBAR SMTP (Para diagn├│stico)
exports.testSmtp = async (req, res) => {
    ...
    try {
        const sent = await enviarCorreo({
            to: targetEmail,
            subject: "Test de Diagn├│stico SMTP - Core360",
            data: {
...
const calcularDefaultCc = async (empresa_id, ejecutiva_id, enviado_por_correo, enviado_por_id) => {
    ...
    // Siempre incluir al remitente en el CC para que tenga su respaldo (env├¡o desde centralizado)
    ...
    // Limpiar duplicados y valores vac├¡os
